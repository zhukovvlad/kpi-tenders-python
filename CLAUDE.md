...
3. `handler(file_bytes, storage_path) -> result_payload: dict`
...
Модуль-специфичная логика живёт в `_handle(file_bytes, storage_path)` внутри `workers/<name>.py`. Чистая функция — без HTTP, без Celery.
