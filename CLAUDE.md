...
- `GeminiAPIError(ValueError)` — постоянная ошибка (неверный ключ, недоступная модель, ValidationError, `ClientError` 4xx). **Не ретраится** — наследует `ValueError`, попадает в `_NO_RETRY`. Классификация через `google.genai.errors`: `ClientError` → `GeminiAPIError`, `ServerError` и прочие → transient (`raise`).
