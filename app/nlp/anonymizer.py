...
_r(
    "LOCATION",
    r"(?<![А-ЯЁа-яёA-Za-z])(?:г\.|город)[ ]?[А-ЯЁ][а-яёА-ЯЁ\-]{2,40}(?:[ ]+[А-ЯЁ][а-яёА-ЯЁ\-]{1,40}){0,2}(?=\s*[,;\n\r]|\s*$|\s+[а-яё])",
)
...
_INN_CONTEXT = frozenset(
    w.lower()
    for w in ["инн", "ИНН", "налогоплательщик", "налоговый номер", "идентификационный номер"]
)
...
class NatashaPipeline:
        ...
        for rx in (_RE_INN_12, _RE_INN_10):
            for m in rx.finditer(text):
                ...
                if not stdnum_inn.is_valid(digits):
                    continue
                if len(digits) == 10 and not self._has_context(
                    text_lower, m.start(), m.end(), _INN_CONTEXT
                ):
                    continue
    ...
    def _has_context(
        ...
        return any(re.search(r"(?<!\w)" + re.escape(w) + r"(?!\w)", window) for w in ctx_words)
